<?php
	...
	if (isset($_GET['id'])){
		$ID=intval($_GET['id']);
		$query=mysqli_query($con, "SELECT * FROM servicios_adicionales WHERE id=".$ID);
		$count=mysqli_num_rows($query);
		if ($count==1){
			if ($delete1=mysqli_query($con,"DELETE FROM servicios_adicionales WHERE id=".$ID)){
			?>
			...
			<div class="alert alert-danger alert-dismissible" role="alert">
			  <button type="button" class="close" data-bs-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			  <strong>Error!</strong> No se pudo eliminar esta  Servicio Adicional.. 
			</div>
			<?php
		}
	...
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
		 ...
		 $aColumns = array('descripcion_servicios_adicionales');//Columnas de busqueda
		 $sTable = "servicios_adicionales";
		 $sWhere = "";
		if ( $_GET['q'] != "" )
		{
			$sWhere = "WHERE (";
		...
		$sWhere.=" ORDER BY descripcion_servicios_adicionales, id";
		...
		$reload = './serviciosAdicionales.php';
		...
		if ($numrows>0){
			...
			<table class="table">
			<tr  class="success">
			<th><font size="2">Servicio Adicional</font></th>
...
echo '<a href="#" class="btn btn-outline-success" title="Editar Servicio Adicional" onclick="obtener_datosServicioAdicional('.$id.');" data-bs-toggle="modal" data-bs-target="#editarServicioAdicional"><i class="bi bi-pencil"></i></a>';

echo '&nbsp;&nbsp;';

echo '<a href="#" class="btn btn-outline-danger" title="Eliminar Servicio Adicional" onclick="eliminar('.$id.')"><i class="bi bi-trash bg-warnig"></i></a>';
