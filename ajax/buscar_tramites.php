<?php
	...
	if (isset($_GET['id'])){
		...
		$query=mysqli_query($con, "SELECT * FROM tramite WHERE id=".$id_tramite);
		$count=mysqli_num_rows($query);
		if ($count==1){
			if ($delete1=mysqli_query($con,"DELETE FROM tramite WHERE id=".$id_tramite)){
			?>
	...
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
		 ...
		 $aColumns = array('descripcion_tramite');//Columnas de busqueda
		 $sTable = "tramite";
		 $sWhere = "";
		if ( $_GET['q'] != "" )
		{
		...
		$sWhere.=" ORDER BY id, descripcion_tramite";
    ...
    if (!empty($q)) {
		...
		<th><font size="1">Tramite</font></th>
		<th><font size="1">Cuenta</font></th>
		<th><font size="1">Monto UMAS</font></th>
		<th><font size="1">Concepto</font></th>
		<th><font size="1">Operación</font></th>
		<th><font size="1">Inspección/RAD/Descuento</font></th>
...
<?php
	...
	$revision_analisis_docs=$row['revision_analisis_docs'];
...
<input type="hidden" value="<?php echo $row['descripcion_tramite'];?>" id="descripcion_tramite<?php echo $id;?>">
...
<input type="hidden" value="<?php echo $fecha;?>" id="fecha<?php echo $id;?>">
	...
	<td><font size="1"><?php echo $id; ?></font></td>
<?php
if ( $descuento=='SI' ) {
echo '<td><font size="2" color="red">'.$descripcion_tramite.'</font></td>';
echo '<td align="center"><font size="1">'.$cuenta.'</font></td>';
echo '<td align="center"><font size="2" color="red">'.$monto_umas.' %</font></td>';
} else {
echo '<td><font size="2">'.$descripcion_tramite.'</font></td>';
echo '<td align="center"><font size="1">'.$cuenta.'</font></td>';
echo '<td align="center"><font size="2">'.$monto_umas.'</font></td>';
}
?>
	<td><font size="1"><?php echo $concepto; ?></font></td>
	<td align="center"><font size="1"><?php echo $operacion; ?></font></td>
	<td align="center"><font size="1"><?php echo $inspeccion.' / '.$revision_analisis_docs.' / '.$descuento; ?></font></td>
	<td class='text-right'>
<?php

echo '<a href="#" class="btn btn-outline-success" title="Editar Tramite" onclick="obtener_datosTramite('.$id.');" data-bs-toggle="modal" data-bs-target="#editarTramite"><i class="bi bi-pencil"></i></a>';

echo '&nbsp;&nbsp;';

echo '<a href="#" class="btn btn-outline-danger" title="Eliminar Tramite" onclick="#eliminar('.$id.')" ><i class="bi bi-trash bg-warnig"></i></a>';
