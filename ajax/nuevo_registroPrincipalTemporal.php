...
#
$arregloSA_musica_grabada_y_aparatos=mysqli_fetch_array(mysqli_query($con,"SELECT monto_umas_permiso_temporal  FROM `servicios_adicionales` WHERE descripcion_servicios_adicionales='Musica Grabada y Aparatos Musicales'"));
...
#
$arregloSA_numero_mesas_de_billar=mysqli_fetch_array(mysqli_query($con,"SELECT monto_umas_permiso_temporal  FROM `servicios_adicionales` WHERE descripcion_servicios_adicionales='Mesas de Billar, por cada Mesa'"));
...
#
$arregloSA_pista_de_baile=mysqli_fetch_array(mysqli_query($con,"SELECT monto_umas_permiso_temporal  FROM `servicios_adicionales` WHERE descripcion_servicios_adicionales='Pista de Baile'"));
$umas_pista_de_baile=$arregloSA_pista_de_baile[0];
$monto_pista_de_baile=$umas_pista_de_baile*$pista_de_baile;
#
$arregloSA_conjunto_musicales=mysqli_fetch_array(mysqli_query($con,"SELECT monto_umas_permiso_temporal  FROM `servicios_adicionales` WHERE descripcion_servicios_adicionales='Conjunto Musicales'"));
...
#
$arregloSA_espectaculos_artisticos=mysqli_fetch_array(mysqli_query($con,"SELECT monto_umas_permiso_temporal  FROM `servicios_adicionales` WHERE descripcion_servicios_adicionales='Espectaculos Artisticos'"));
...
$arregloTramite=mysqli_fetch_array(mysqli_query($con,"SELECT *  FROM `tramite` WHERE descripcion_tramite='Permiso Temporal'"));
$ID_TRAMITE=$arregloTramite[0];
###############
###############
###
##########
$sql_principal="INSERT INTO principal_temp (
giro,
...
$arregloMaxid = mysqli_fetch_array(mysqli_query($con,"SELECT max(`id`) FROM `principal_temp`"));
$ID=intval($arregloMaxid[0]);
$folio=$ID_MUNICIPIO."-".$ID;
############
############
$sqlInsert="INSERT INTO proceso_tramites_temp (
id_principal,
id_tramite,
fecha_inicio,
fechaRegistro,
...
##
$arregloMaxid2 = mysqli_fetch_array(mysqli_query($con,"SELECT max(`id`) FROM `proceso_tramites_temp`"));
$ID_PROCESO_TRAMITE=$arregloMaxid2[0];
##########################
### SERVICIOS ADICIONALES

if ( $SERVICIOS_ADICIONALES_LISTA=='0' ) {
$Kuery_SAP="INSERT INTO servicios_adicionales_permisionario_temp (
id_principal,
id_proceso_tramites,
folio,
...
####
$arregloMaxid3 = mysqli_fetch_array(mysqli_query($con,"SELECT max(`id`) FROM `servicios_adicionales_permisionario_temp`"));
...
$Kuery_Update="UPDATE principal_temp SET folio='$folio', id_servicios_adicionales_permisionario=".$ID_SERVICIOS_ADICIONALES_PERMISIONARIO.", id_tramite=$ID_TRAMITE , id_proceso_tramites=$ID_PROCESO_TRAMITE  WHERE id=".$ID;
mysqli_query($con,$Kuery_Update);
##########################
########################
########################
##
$sql_tramite0="SELECT * FROM tramite WHERE descripcion_tramite='Inspeccion - Permiso Temporal'";
...
##
$sql_tramite00="SELECT * FROM tramite WHERE descripcion_tramite='Recepcion y Analisis Documentos - Permiso Temporal'";
...
##
$sql40="INSERT INTO recepcion_analisis_documentos_temp (
id_principal,
folio,
id_proceso_tramites,
fechaRegistro,
