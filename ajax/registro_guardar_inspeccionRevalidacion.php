...
id_principal=$ID AND id_proceso_tramites=0 AND observaciones='En Proceso Revalidacion'";
...
##if ( $cuentaSTATUS> 0 ) {
$Kuery_Update="UPDATE principal SET latitud='".$latitud."', longitud='".$longitud."', estatus='Presupuesto' WHERE id=".$ID;
##} else {
...
mysqli_close($con);
			...
			if ($query_Update) {
				$messages[] = "Se Finalizo la  Inspección-Revalidación con Exito Folio ($FOLIO )";
			} else {
