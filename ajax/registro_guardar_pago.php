...
##
$sqlUpdate_pago="UPDATE pagos SET total_umas_pagar='$total_umas_pagar', numero_pago='$numero_pago', monto='$monto', concepto_pago='$tramite_pagoid ** $nombre_comercial_establecimiento', fecha_pago='$today', estatus_pago='PAGADO'  WHERE id=$id_pago_rad";
##
$Kuery_Update_principal="UPDATE principal SET estatus='Pago RAD' WHERE id=".$ID;
        break;
    case "Inspeccion":
##
$sqlUpdate_pago="UPDATE pagos SET total_umas_pagar='$total_umas_pagar', numero_pago='$numero_pago', monto='$monto', concepto_pago='$tramite_pagoid ** $nombre_comercial_establecimiento', fecha_pago='$today', estatus_pago='PAGADO'  WHERE id=$id_pago_ins";
##
$Kuery_Update_principal="UPDATE principal SET estatus='Pago INS' WHERE id=".$ID;
        break;
    default:
$Kuery_Update_principal="UPDATE principal SET estatus='PAGO X' WHERE id=".$ID;
}
...
$PAGO_INS=0;
###
$sql_pagoRAD="SELECT * FROM `pagos` WHERE `id_principal`=$ID AND `concepto`='Recepcion y Analisis Documentos' AND estatus_pago='PAGADO' AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
...
##
$sql_pagoI="SELECT * FROM `pagos` WHERE `id_principal`=$ID AND `concepto`='Inspeccion' AND `estatus_pago`='PAGADO' AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
