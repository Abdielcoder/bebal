// Variables globales
let dropzoneInstance_ct1;
let uploadingFiles_ct1 = 0;

// Función para inicializar el modal de PDF para un registro específico
function pdf_registro1file_ct1(id,conjunto,folio,id_proceso_tramites,page) {
...
//alert("id="+id+", conjunto="+conjunto+", folio="+folio+", id_proceso_tramites="+id_proceso_tramites);
    ...
    $('#pdf_id_registro_ct1').val(id);
    $('#pdf_conjunto_ct1').val(conjunto);
    $('#pdf_folio_ct1').val(folio);
    $('#pdf_id_proceso_tramites_ct1').val(id_proceso_tramites);
    $('#page_ct1').val(page);
    
    // Limpiar resultados previos
    $('#resultados_pdf_ct1').html('');
    $('#pdf_archivos_manual_ct1').val('');
    $('#upload-progress-container_ct1').addClass('d-none');
    $('#upload-progress-bar_ct1').css('width', '0%').attr('aria-valuenow', 0);
    
    // Desactivar la auto-detección para manejar manualmente
    Dropzone.autoDiscover = false;
    
    // Destruir instancia previa si existe
    if (dropzoneInstance_ct1) {
        dropzoneInstance_ct1.destroy();
    ...
    dropzoneInstance_ct1 = new Dropzone("#mi-dropzone", {
        ...
        init: function() {
            ...
            this.on("sending", function(file, xhr, formData) {
                ...
                $('#upload-progress-container_ct1').removeClass('d-none');
                $('#resultados_pdf_ct1').html('<div class="alert alert-info">Subiendo archivos... <div class="loading-spinner"></div></div>');
            ...
            this.on("uploadprogress", function(file, progress) {
                ...
                $('#upload-progress-bar_ct1').css('width', progressValue + '%').attr('aria-valuenow', progressValue);
            ...
            this.on("success", function(file, response) {
                ...
                if (uploadingFiles_ct1 === 0) {
                    $('#resultados_pdf_ct1').html('<div class="alert alert-success">Todos los archivos se han subido correctamente.</div>');
                    cargarPDFs(id);
            ...
            this.on("error", function(file, errorMessage) {
                ...
                if (uploadingFiles_ct1 === 0) {
                    $('#resultados_pdf_ct1').html('<div class="alert alert-danger">Ha ocurrido un error al subir algunos archivos.</div>');
                    cargarPDFs(id);
...
$(document).on('click', '#btn-subir-pdf-manual_ct1', function() {
    const archivos = $('#pdf_archivos_manual_ct1')[0].files;
    const id_registro = $('#pdf_id_registro_ct1').val();
    const conjunto = $('#pdf_conjunto_ct1').val();
    const folio = $('#pdf_folio_ct1').val();
    const id_proceso_tramites = $('#pdf_id_proceso_tramites_ct1').val();
    const page = $('#page_ct1').val();
...
//alert("btn-subir-pdf-manual--> id="+id_registro+", conjunto="+conjunto+", folio="+folio+", id_proceso_tramites="+id_proceso_tramites);
    ...
    if (archivos.length === 0) {
        $('#resultados_pdf_ct1').html('<div class="alert alert-warning">Debe seleccionar un archivo PDF.</div>');
        return;
    }
    
    // Mostrar mensaje de carga
    $('#resultados_pdf_ct1').html('<div class="alert alert-info">Preparando archivo para subir... <div class="loading-spinner"></div></div>');
    ...
    for (let i = 0; i < archivos.length; i++) {
        ...
        $('#upload-progress-bar_ct1').css('width', progresoInicial + '%').attr('aria-valuenow', progresoInicial);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#upload-progress-bar_ct1').css('width', progreso + '%').attr('aria-valuenow', progreso);
                ...
                if ((archivosSubidos + archivosConError) === archivos.length) {
                    ...
                    if (archivosConError === 0) {
                        $('#resultados_pdf_ct1').html('<div class="alert alert-success">El archivo ha subido correctamente. Espere un Momento  la ventana se cerrará Automáticamente..</div>');
                    } else if (archivosSubidos === 0) {
                        $('#resultados_pdf_ct1').html('<div class="alert alert-danger">No se pudo subir ningún archivo. Por favor, inténtelo nuevamente.</div>');
                    } else {
                        $('#resultados_pdf_ct1').html('<div class="alert alert-warning">Se subieron ' + archivosSubidos + ' archivos, pero ' + archivosConError + ' no se pudieron subir.</div>');
                    }
            ...
            error: function() {
                ...
                if ((archivosSubidos + archivosConError) === archivos.length) {
                    ...
                    if (archivosSubidos === 0) {
                        $('#resultados_pdf_ct1').html('<div class="alert alert-danger">Error al subir los archivos. Por favor, inténtelo nuevamente.</div>');
                    } else {
                        $('#resultados_pdf_ct1').html('<div class="alert alert-warning">Se subieron ' + archivosSubidos + ' archivos, pero ' + archivosConError + ' no se pudieron subir.</div>');
                    }
