// Variables globales
let dropzoneInstance_ct4;
let uploadingFiles_ct4 = 0;

// Función para inicializar el modal de PDF para un registro específico
function pdf_registro1file_ct4(id,conjunto,folio,id_proceso_tramites,page) {
...
//alert("id="+id+", conjunto="+conjunto+", folio="+folio+", id_proceso_tramites="+id_proceso_tramites);
    ...
    $('#pdf_id_registro_ct4').val(id);
    $('#pdf_conjunto_ct4').val(conjunto);
    $('#pdf_folio_ct4').val(folio);
    $('#pdf_id_proceso_tramites_ct4').val(id_proceso_tramites);
    $('#page_ct4').val(page);
    
    // Limpiar resultados previos
    $('#resultados_pdf_ct4').html('');
    $('#pdf_archivos_manual_ct4').val('');
    $('#upload-progress-container_ct4').addClass('d-none');
    $('#upload-progress-bar_ct4').css('width', '0%').attr('aria-valuenow', 0);
    
    // Desactivar la auto-detección para manejar manualmente
    Dropzone.autoDiscover = false;
    
    // Destruir instancia previa si existe
    if (dropzoneInstance_ct4) {
        dropzoneInstance_ct4.destroy();
    ...
    dropzoneInstance_ct4 = new Dropzone("#mi-dropzone", {
        ...
        init: function() {
            ...
            this.on("sending", function(file, xhr, formData) {
                ...
                $('#upload-progress-container_ct4').removeClass('d-none');
                $('#resultados_pdf_ct4').html('<div class="alert alert-info">Subiendo archivos... <div class="loading-spinner"></div></div>');
            ...
            this.on("uploadprogress", function(file, progress) {
                ...
                $('#upload-progress-bar_ct4').css('width', progressValue + '%').attr('aria-valuenow', progressValue);
            ...
            this.on("success", function(file, response) {
                ...
                if (uploadingFiles_ct4 === 0) {
                    $('#resultados_pdf_ct4').html('<div class="alert alert-success">Todos los archivos se han subido correctamente.</div>');
                    cargarPDFs(id);
            ...
            this.on("error", function(file, errorMessage) {
                ...
                if (uploadingFiles_ct4 === 0) {
                    $('#resultados_pdf_ct4').html('<div class="alert alert-danger">Ha ocurrido un error al subir algunos archivos.</div>');
                    cargarPDFs(id);
...
$(document).on('click', '#btn-subir-pdf-manual_ct4', function() {
    const archivos = $('#pdf_archivos_manual_ct4')[0].files;
    const id_registro = $('#pdf_id_registro_ct4').val();
    const conjunto = $('#pdf_conjunto_ct4').val();
    const folio = $('#pdf_folio_ct4').val();
    const id_proceso_tramites = $('#pdf_id_proceso_tramites_ct4').val();
    const page = $('#page_ct4').val();
...
//alert("btn-subir-pdf-manual--> id="+id_registro+", conjunto="+conjunto+", folio="+folio+", id_proceso_tramites="+id_proceso_tramites);
    ...
    if (archivos.length === 0) {
        $('#resultados_pdf_ct4').html('<div class="alert alert-warning">Debe seleccionar un archivo PDF.</div>');
        return;
    }
    
    // Mostrar mensaje de carga
    $('#resultados_pdf_ct4').html('<div class="alert alert-info">Preparando archivo para subir... <div class="loading-spinner"></div></div>');
    ...
    for (let i = 0; i < archivos.length; i++) {
        ...
        $('#upload-progress-bar_ct4').css('width', progresoInicial + '%').attr('aria-valuenow', progresoInicial);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#upload-progress-bar_ct4').css('width', progreso + '%').attr('aria-valuenow', progreso);
                ...
                if ((archivosSubidos + archivosConError) === archivos.length) {
                    ...
                    if (archivosConError === 0) {
                        $('#resultados_pdf_ct4').html('<div class="alert alert-success">El archivo ha subido correctamente. Espere un Momento  la ventana se cerrará Automáticamente..</div>');
                    } else if (archivosSubidos === 0) {
                        $('#resultados_pdf_ct4').html('<div class="alert alert-danger">No se pudo subir ningún archivo. Por favor, inténtelo nuevamente.</div>');
                    } else {
                        $('#resultados_pdf_ct4').html('<div class="alert alert-warning">Se subieron ' + archivosSubidos + ' archivos, pero ' + archivosConError + ' no se pudieron subir.</div>');
                    }
            ...
            error: function() {
                ...
                if ((archivosSubidos + archivosConError) === archivos.length) {
                    ...
                    if (archivosSubidos === 0) {
                        $('#resultados_pdf_ct4').html('<div class="alert alert-danger">Error al subir los archivos. Por favor, inténtelo nuevamente.</div>');
                    } else {
                        $('#resultados_pdf_ct4').html('<div class="alert alert-warning">Se subieron ' + archivosSubidos + ' archivos, pero ' + archivosConError + ' no se pudieron subir.</div>');
                    }
