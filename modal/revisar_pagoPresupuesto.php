...
function procesando_PP1() {
  var element = document.getElementById("Respuesta_PP1");
  element.classList.remove("content2_PP1");
  element.classList.toggle("content_PP1");
  element.classList.toggle("blink_PP1");
document.getElementById("Respuesta_PP1").innerHTML = "Procesando Consulta Sistema Tesoreria .......";
}

function finalizado_PP1() {
document.getElementById("mensaje").innerHTML = "Finalizado";
}


function getRecibo_PP1(forma) {
...
timeout: 5000,
  ...
  var Respuesta_PP1=mensaje.nodeValue+" $"+importe.nodeValue+" estatus ("+idstatus.nodeValue+")";

  var element = document.getElementById("Respuesta_PP1");
  element.classList.remove("blink_PP1");
  element.classList.toggle("content2_PP1");

  document.getElementById("Respuesta_PP1").innerHTML = Respuesta_PP1;
  },
error: function(){
  var element = document.getElementById("Respuesta_PP1");
  element.classList.remove("blink_PP1");
  element.classList.toggle("content3_PP1");

  document.getElementById("Respuesta_PP1").innerHTML = 'Fallo Consulta del Recibo de Pago';
...
?>
	<!-- Modal -->
        <div class="modal fade" id="revisarPagoPresupuesto" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">

	  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header"  style="background-color:#AC905B;color:white">
			<h6 class="modal-title" id="revisarPagoLabel"><i class='bi bi-credit-card'></i> Permiso Nuevo Presupuesto</h6>
<!-- <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>  --!>
		  </div>
		  <div class="modal-body">
			<form class="form-horizontal" method="post" id="registro_guardar_pago_presupuesto" name="registro_guardar_pago_presupuesto">
			<div id="resultados_ajaxPagoPresupuesto"></div>
<?php
...
echo '<a href="javascript:getRecibo_PP1(document.registro_guardar_pago_presupuesto)" style="background-color:#000000;color:white"><font size="1">Consultar Recibo</font></a>';
echo '</div>';
...
echo '<div id="Respuesta_PP1" style="color:black;font-size:12px;"></div>';
echo '</div>';
...
<button type="submit" class="btn btn-primary" id="Button_registro_guardar_pago_presupuesto"  style="background-color:#AC905B;color:black"> Registrar Pago Presupuesto</button>
</div>
