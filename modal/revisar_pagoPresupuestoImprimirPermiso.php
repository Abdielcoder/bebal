...
function procesando_PP2() {
  var element = document.getElementById("Respuesta_PP2");
  element.classList.remove("content2_PP2");
  element.classList.toggle("content_PP2");
  element.classList.toggle("blink_PP2");
document.getElementById("Respuesta_PP2").innerHTML = "Procesando Consulta Sistema Tesoreria .......";
}

function finalizado_PP2() {
document.getElementById("mensaje").innerHTML = "Finalizado";
}


function getRecibo_PP2(forma) {
...
timeout: 5000,
  ...
  var Respuesta_PP2=mensaje.nodeValue+" $"+importe.nodeValue+" estatus ("+idstatus.nodeValue+")";

  var element = document.getElementById("Respuesta_PP2");
  element.classList.remove("blink_PP2");
  element.classList.toggle("content2_PP2");

  document.getElementById("Respuesta_PP2").innerHTML = Respuesta_PP2;
  },
error: function(){
  var element = document.getElementById("Respuesta_PP2");
  element.classList.remove("blink_PP2");
  element.classList.toggle("content3_PP2");

  document.getElementById("Respuesta_PP2").innerHTML = 'Fallo Consulta del Recibo de Pago';
...
?>
	<!-- Modal -->
        <div class="modal fade" id="revisarPagoPresupuestoImprimirPermiso" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">

	  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header"  style="background-color:#AC905B;color:white">
			<h6 class="modal-title" id="revisarPagoLabel"><i class='bi bi-credit-card'></i> Pago Impresión de Permiso</h6>
<!-- <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>  --!>
		  </div>
		  <div class="modal-body">
			<form class="form-horizontal" method="post" id="registro_guardar_pago_presupuestoImprimirPermiso" name="registro_guardar_pago_presupuestoImprimirPermiso">
			<div id="resultados_ajaxPagoPresupuestoImprimirPermiso"></div>
<?php
...
echo '<a href="javascript:getRecibo_PP2(document.registro_guardar_pago_presupuestoImprimirPermiso)" style="background-color:#000000;color:white"><font size="1">Consultar Recibo</font></a>';
echo '</div>';
...
echo '<div id="Respuesta_PP2" style="color:black;font-size:12px;"></div>';
echo '</div>';
...
<button type="submit" class="btn btn-primary" id="Button_registro_guardar_pago_presupuestoImprimirPermiso"  style="background-color:#AC905B;color:black"> Registrar Pago</button>
</div>
