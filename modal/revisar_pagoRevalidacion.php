...
function procesando_RV() {
  var element = document.getElementById("Respuesta_RV");
  element.classList.remove("content2_RV");
  element.classList.toggle("content_RV");
  element.classList.toggle("blink_RV");
document.getElementById("Respuesta_RV").innerHTML = "Procesando Consulta Sistema Tesoreria .......";
}

function finalizado_RV() {
document.getElementById("mensaje").innerHTML = "Finalizado";
}


function getRecibo_RV(forma) {
...
timeout: 5000,
  ...
  var Respuesta_RV=mensaje.nodeValue+" $"+importe.nodeValue+" estatus ("+idstatus.nodeValue+")";

  var element = document.getElementById("Respuesta_RV");
  element.classList.remove("blink_RV");
  element.classList.toggle("content2_RV");

  document.getElementById("Respuesta_RV").innerHTML = Respuesta_RV;
  },
error: function(){
  var element = document.getElementById("Respuesta_RV");
  element.classList.remove("blink_RV");
  element.classList.toggle("content3_RV");

  document.getElementById("Respuesta_RV").innerHTML = 'Fallo Consulta del Recibo de Pago';
...
?>
	<!-- Modal -->
        <div class="modal fade" id="revisarPagoTramiteRevalidacion" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">

	  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header"  style="background-color:#AC905B;color:white">
			<h6 class="modal-title" id="revisarPagoTramiteCambioLabel"><i class='bi bi-credit-card'></i> Revisar y Registrar Pago - Revalidación</h6>
<!-- <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>  --!>
		  </div>
		  <div class="modal-body">
			<form class="form-horizontal" method="post" id="registro_guardar_pagoTramiteRevalidacion" name="registro_guardar_pagoTramiteRevalidacion">
			<div id="resultados_ajaxPagoTramiteRevalidacion"></div>
<?php
...
echo '<a href="javascript:getRecibo_RV(document.registro_guardar_pagoTramiteRevalidacion)" style="background-color:#000000;color:white"><font size="1">Consultar Recibo</font></a>';
echo '</div>';
...
echo '<div id="Respuesta_RV" style="color:black;font-size:12px;"></div>';
echo '</div>';
...
<button type="submit" class="btn btn-primary" id="Button_registro_guardar_pagoTramiteRevalidacion"  style="background-color:#AC905B;color:black"> Registrar Pago </button>
</div>
