...
$id_cierre_temporal=$_GET['id_cierre_temporal'];
} else {
...
include("modal/efectuar_inspeccionCierreTemporal.php");
...
$latitudDB=$arregloPRINCIPAL['latitud'];
$longitudDB=$arregloPRINCIPAL['longitud'];
################
...
################
#chang
$KueryPT_FINALIZADO= "SELECT COUNT(*)  AS cuentaFINALIZADO FROM inspeccion WHERE en_proceso='FIN' AND id_principal=$IDPRINCIPAL AND id_proceso_tramites=$id_cierre_temporal AND observaciones='Cierre Temporal'";
...
echo "<h6><b><span style='background:#AC905B;'><font color='white'>Inspección - Revalidación del Permiso: </span></b><font color='black'> ".$folioDB.", ".$nombre_comercial_establecimientoDB." (".$GIRO."), ".$domicilio_establecimiento."</font></h5>";
##############################
/* Agregar foto */
$ret="";
if (!isset($errorfoto)) $errorfoto="";

$ret .= '<form ENCTYPE="multipart/form-data" name="subirnuevafoto" action="principalFotosCierreTemporal.php" method="POST">';
...
$ret .= "<input type=\"hidden\" name=\"id_cierre_temporal\" value=\"".$id_cierre_temporal."\">";
...
echo '<input type="hidden" name="id_cierre_temporal" value="'.$id_cierre_temporal.'">';
...
echo '<input type="hidden" id="id_cierre_temporal" value="'.$id_cierre_temporal.'" >';
...
echo '<input type="hidden" id="latitud" value="'.$latitudDB.'" >';
echo '<input type="hidden" id="longitud" value="'.$longitudDB.'" >';
...
echo '<a href="#" class="btn btn-danger btn-sm" title="Efectuar Inspeccion" onclick="obtener_datosInspeccion('.$IDPRINCIPAL.','.$id_cierre_temporal.');" data-bs-toggle="modal" data-bs-target="#EfectuarInspeccionCierreTemporal"><i class="bi bi-gear"></i><font size="1"> Registrar Inspección </font></a>';
}
...
$( "#registro_guardar_inspeccionCierreTemporal" ).submit(function( event ) {
  $('#Button_registro_guardar_inspeccionCierreTemporal').attr("disabled", true);

 var parametros = $(this).serialize();
         $.ajax({
                        type: "POST",
                        url: "ajax/registro_guardar_inspeccionCierreTemporal.php",
                        data: parametros,
                         beforeSend: function(objeto){
                                $("#resultados_ajaxInspeccionCierreTemporal").html("Mensaje: Cargando...");
                          },
                        success: function(datos){
                        $("#resultados_ajaxInspeccionCierreTemporal").html(datos);
                        $('#Button_registro_guardar_inspeccionCierreTemporal').attr("disabled", true);
...
function obtener_datosInspeccion(id,id_cierre_temporal){

        var id = $("#id").val();
        var id_cierre_temporal = $("#id_cierre_temporal").val();
        ...
        $("#mod_id").val(id);
        $("#mod_id_cierre_temporal").val(id_cierre_temporal);
        ...
        $("#mod_longitud").val(longitud);
        $("#mod_latitud").val(latitud);
