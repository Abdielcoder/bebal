...
$queryINSERTA="INSERT INTO fotos_temp (descripcion,idprincipal,id_proceso_tramites) VALUES ('Inserta Row, idprincipal=".$IDPRINCIPAL."',".$IDPRINCIPAL.",".$ID_PROCESO_TRAMITES.");";
if (!mysqli_query($con,$queryINSERTA)) echo mysqli_error();
$queryMAXidfoto="SELECT MAX(idfoto) AS idfoto FROM fotos_temp;";
...
$queryUPDATE="UPDATE fotos_temp SET descripcion='OK' WHERE idfoto=".$nextidf." AND idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
if (!mysqli_query($con,$queryUPDATE)) echo mysqli_error();
...
$sqlFotosCuantosReg="SELECT * FROM fotos_temp WHERE idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
...
$sqlUpdate1="UPDATE principal_temp SET foto='$nextidf' WHERE id=".$IDPRINCIPAL;
...
$sqlFotosCuantosReg1="SELECT * FROM fotos_temp WHERE idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
...
for ($i=0;$i<$rowsFotosCuantosReg1;$i++) {
		...
		##echo "IDFOTO=".$IDFOTO;
		$sqlUpdate2="UPDATE principal_temp SET foto='$IDFOTO' WHERE id=".$IDPRINCIPAL;
		##echo $sqlUpdate2;
...
#################################
// Eliminar
$sqlFotosCuantosReg2="SELECT * FROM fotos_temp WHERE idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
...
for ($i=0;$i<$rowsFotosCuantosReg2;$i++) {
	if (isset($_POST['eliminar'.$i])) {
		$IDFOTO=$_POST['idfoto'.$i];
		$sqlUpdate2="DELETE FROM fotos_temp WHERE idfoto=".$IDFOTO;
		mysqli_query($con, $sqlUpdate2);
// Si hay una sola la establece como principal
$sqlFotosCuantosReg="SELECT * FROM fotos_temp WHERE idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
$resultFotosCuantosReg = mysqli_query($con, $sqlFotosCuantosReg);
...
$sqlUpdate4="UPDATE principal_temp SET foto=NULL WHERE id=".$IDPRINCIPAL;
mysqli_query($con, $sqlUpdate4);
} else {
$sqlFotos1="SELECT * FROM fotos_temp WHERE idprincipal=".$IDPRINCIPAL." AND id_proceso_tramites=".$ID_PROCESO_TRAMITES;
...
$sqlUpdate3="UPDATE principal_temp SET foto='$IDFOTO_DB' WHERE id=".$IDPRINCIPAL;
...
include("modal/efectuar_inspeccionTemp.php");
...
##
################
$KueryPT_FINALIZADO= "SELECT COUNT(*)  AS cuentaFINALIZADO FROM inspeccion_temp WHERE en_proceso='FIN' AND id_principal=$IDPRINCIPAL AND id_proceso_tramites=$id_proceso_tramites";
...
echo "<h6><b><span style='background:#AC905B;'><font color='white'>Permiso Temporal - Inspección:</span></b><font color='black'> ".$folioDB.", ".$nombre_comercial_establecimientoDB." (".$GIRO."), ".$domicilio_establecimiento."</font></h5>";
##############################
/* Agregar foto */
$ret="";
if (!isset($errorfoto)) $errorfoto="";

$ret .= '<form ENCTYPE="multipart/form-data" name="subirnuevafoto" action="principalFotosTemporal.php" method="POST">';
...
echo '<br><br>';
##############################
$foto_file='../'.FOTOSMEDIAS.'T-'.$IDPRINCIPAL.'-'.$ID_PROCESO_TRAMITES.'-'.$foto_principalDB.'.jpg';
...
echo "</table>";
//chang
$sqlFotos="SELECT * FROM fotos_temp f WHERE f.idprincipal=".$IDPRINCIPAL." AND f.id_proceso_tramites=".$ID_PROCESO_TRAMITES." ORDER BY f.idfoto;";
echo $sqlFotos;
...
#########
echo "<td align=center>";
$foto_fileSecundaria='../'.FOTOSMEDIAS.'T-'.$IDPRINCIPAL.'-'.$ID_PROCESO_TRAMITES.'-'.$idfoto_db.'.jpg';
...
echo '<br>';
echo '<FORM action="principal_temp.php" name="ir_aPrincipal" method="POST">';
...
echo '<a href="#" class="btn btn-danger btn-sm" title="Efectuar Inspeccion - Permiso Temporal" onclick="obtener_datosInspeccion('.$IDPRINCIPAL.');" data-bs-toggle="modal" data-bs-target="#EfectuarInspeccionTemp"><i class="bi bi-gear"></i><font size="1"> Registrar Inspección </font></a>';
}
...
$( "#registro_guardar_inspeccionTemp" ).submit(function( event ) {
  $('#Button_registro_guardar_inspeccionTemp').attr("disabled", true);

 var parametros = $(this).serialize();
         $.ajax({
                        type: "POST",
                        url: "ajax/registro_guardar_inspeccionTemporal.php",
                        data: parametros,
                         beforeSend: function(objeto){
                                $("#resultados_ajaxInspeccionTemp").html("Mensaje: Cargando...");
                          },
                        success: function(datos){
                        $("#resultados_ajaxInspeccionTemp").html(datos);
                        $('#Button_registro_guardar_inspeccionTemp').attr("disabled", true);
